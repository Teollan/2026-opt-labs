...
for example in examples:
    ...
    result = subprocess.run(
        ["./build/apps/lab2", "-s", str(example), "-t", "-T", "-i", "-l"],
...
output_dir = Path("docs/lab2")
output_dir.mkdir(parents=True, exist_ok=True)
output = sys.argv[1] if len(sys.argv) > 1 else str(output_dir / "Козлов КВ-33 ЛАБ2 Додаток 2.pdf")
HTML(string=html).write_pdf(output)
print("Generated tests PDF in docs/lab2/")
